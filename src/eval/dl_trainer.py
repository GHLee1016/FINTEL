...
from src.preprocess.sequence import build_loaders, make_sequences, make_loader
...
def _search_space(trial: optuna.Trial, model_name: str) -> dict[str, Any]:
    ...
    if model_name == '1DCNN':
        ...
        # Mamba는 hidden_dim 대신 d_model 사용
        params.pop('hidden_dim')
        params['d_model']    = trial.suggest_categorical('d_model',  [32, 64, 128])
        params['d_state']    = trial.suggest_categorical('d_state',  [8, 16, 32])
        params['d_conv']     = trial.suggest_categorical('d_conv',   [2, 4])
        params['expand']     = trial.suggest_categorical('expand',   [1, 2])
        params['num_layers'] = trial.suggest_int('num_layers', 1, 3)
...
def run_dl_expanding(
    ...
    best_params : dict[str, Any],
    refit_every : int   = 5,
    max_epochs  : int   = 30,     # expanding은 빠르게
    patience    : int   = 5,
    batch_size  : int   = 64,
    seed        : int   = 42,
    device      : torch.device = DEVICE,
    ...
    for i, (date, row) in enumerate(test_df.iterrows()):
        ...
        if i % refit_every == 0:
            ...
            tr_loader, va_loader, _, _ = build_loaders(
                inner_tr, inner_va, inner_va,   # test 자리 dummy
                feature_cols, seq_len, batch_size,
            )
