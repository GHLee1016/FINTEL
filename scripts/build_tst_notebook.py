"""One-shot generator for notebooks/08_tst.ipynb.
...
CELLS = [
    ...
    md(
        "# 08 — TST 모델 학습 / 평가",
        "",
        "DL Transformer 모델. dataset_DL/의 사전 변환된 sliding window 데이터로 TST 학습.",
        ...
        "- `results/best_dl_models/TST_{regime}_{country}_{tier}_L{L}.pt` — 학습된 state_dict",
        "- `log/TST/log_TST_{regime}_{country}.csv` — 시계열 예측 (ML과 동일 형식)",
        "",
        "## 실행 흐름",
    ...
    md(
        ...
        "| Activation | GELU | Transformer 표준 (Vaswani 이후 BERT/GPT) |",
        "| d_model | 64 | LSTM hidden_size와 동일 — 파라미터 수 비교 가능 |",
        "| nhead | 4 | head_dim = 16 (표준) |",
        "| num_layers (encoder) | 2 | 작은 dataset에서 sweet spot. 깊으면 overfit |",
        "| dim_feedforward | 128 | d_model × 2 (보수적) |",
        "| Dropout | 0.2 | embed, MHA, FFN 전부 동일 |",
        "| norm_first | True | pre-norm (작은 dataset 학습 안정) |",
        "| Positional encoding | Sinusoidal | parameter-free, L 다양해도 generalize |",
        "| Causal mask | 없음 | window 자체에 미래 정보 없음 |",
        "| Batch size | 64 | shuffle=False (시계열) |",
        "| Max epochs | 100 + early stop patience 10 | |",
        "| Gradient clip | norm 1.0 | gradient explosion 방지 |",
    ...
    code(
        ...
        "    dropout=0.2,",
        "    activation='gelu',",
        "    norm_first=True,           # pre-norm (작은 dataset 안정)",
        "    max_len=300,               # L=252 + 여유",
        "    # optimizer",
        "    lr=1e-3,",
        "    weight_decay=1e-5,",
        "    batch_size=64,",
    ...
    # ----- Cell 5: TST 모델 설명 -----
    md(
        "## TST 구조 (Encoder-only Transformer)",
        "",
        "```",
        "Input (B, L, F)",
        "  ↓",
        "Linear(F → d_model=64)              # input embedding",
        "  ↓",
        "+ SinusoidalPositionalEncoding      # max_len=300, parameter-free",
        "  ↓",
        "Dropout(0.2)",
        "  ↓",
        "TransformerEncoderLayer × 2         # pre-norm, batch_first, gelu",
        "  ├─ LayerNorm → MHA(nhead=4) → Dropout → +residual",
        "  └─ LayerNorm → FFN(64→128→64) → Dropout → +residual",
        "  ↓",
        "LayerNorm                            # final norm (pre-norm 시 필수)",
        "  ↓",
        "out[:, -1, :] → (B, 64)              # 마지막 timestep (LSTM/TCN/CNN1D와 동일)",
        "  ↓",
        "Linear(64 → 1) → squeeze → (B,)      # 다음날 RV 예측",
        "```",
        "",
        "- **Encoder-only**: regression task(sequence-to-one)라 decoder 불필요.",
        "- **Sinusoidal PE**: parameter-free, L=22/60/252 모두 generalize.",
        "- **Pre-norm**: 작은 dataset (위기 cell 76개)에서 학습 안정. residual이 LN 거치지 않고 직접 흐름.",
        "- **마지막 timestep head**: RV lag-1 신호 직접 활용. AvgPool/[CLS] 모두 비추.",
        "- **Causal mask 없음**: input window 자체가 sliding window로 미래 정보 차단된 상태.",
        "- **Receptive field = L (전체)**: self-attention이라 모든 timestep을 봄.",
        "",
        "Trainable params: ~67K (LSTM 25K, CNN1D 30K, TCN 80K와 비교).",
        "",
        "정의는 `src/models/dl/tst.py` 의 `TSTNet` + `TSTModel`.",
    ),
    # ----- Cell 6: 모델 인터페이스 확인 -----
    code(
        "# TSTModel은 src/models/dl/tst.py에서 import 완료",
        "print(TSTModel.__doc__.splitlines()[0])",
        "print(f'name = {TSTModel.name}')",
    ),
    # ----- Cell 7: helper 설명 -----
    md(
        "## 헬퍼",
    ...
    code(
        ...
        "            ckpt_path = DL_MODELS_DIR / f'TST_{regime}_{country}_{tier}.pt'",
    ...
    md(
        ...
        "- 시계열 log는 (regime, country)별 1 파일 — `log/TST/log_TST_{regime}_{country}.csv`",
    ),
    # ----- Cell 12: save code -----
    code(
    ...
    md(
        ...
        "- TST full retrain (early stop 활성)",
        "",
        "**가볍게 시작**: `REFIT_EVERY=50`으로 키우면 refit 횟수 절반.",
    ),
    # ----- Cell 14: EXPANDING LOOP -----
    code(
        "REFIT_EVERY = 20      # TST 학습 비쌈 (attention O(L²)). ML(5)보다 큰 값",
        "",
        ...
        "                ckpt_path = DL_MODELS_DIR / f'TST_{regime}_{country}_{tier}_L{L}_expanding.pt'",
    ...
    code(
        ...
        "    print('\\n=== ML(core) vs TST(core) Full Test best ===')",
...
def build():
    ...
    out = Path(__file__).resolve().parents[1] / "notebooks" / "08_tst.ipynb"
