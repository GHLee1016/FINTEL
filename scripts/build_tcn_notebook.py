"""One-shot generator for notebooks/07_tcn.ipynb.
...
CELLS = [
    ...
    md(
        "# 07 — TCN 모델 학습 / 평가",
        "",
        "DL 첫 모델. dataset_DL/의 사전 변환된 sliding window 데이터로 TCN 학습.",
        ...
        "- `results/best_dl_models/TCN_{regime}_{country}_{tier}_L{L}.pt` — 학습된 state_dict",
        "- `log/TCN/log_TCN_{regime}_{country}.csv` — 시계열 예측 (ML과 동일 형식)",
        "",
        "## 실행 흐름",
    ...
    # ----- Cell 5: TCN 모델 설명 -----
    md(
        "## TCN 구조 (Bai et al., 2018)",
        "",
        "```",
        "Input (B, L, F)",
        "  ↓ transpose(1, 2) → (B, F, L)  # Conv1d format",
        "  ↓",
        "TemporalBlock(F → 64, dilation=1):",
        "    CausalConv1d(K=3, d=1) → ReLU → Dropout",
        "    CausalConv1d(K=3, d=1) → ReLU → Dropout + residual",
        "TemporalBlock(64 → 64, dilation=2): …",
        "TemporalBlock(64 → 64, dilation=4): …",
        "  ↓ (B, 64, L)",
        "out[:, :, -1] → (B, 64)        # 마지막 timestep (causal)",
        "  ↓",
        "nn.Linear(64 → 1) → squeeze → (B,)",
        "```",
        "",
        "- **Causal conv**: padding을 좌측에만, 우측은 chomp로 잘라 미래 정보 차단",
        "- **Dilation 지수 증가** (1, 2, 4, …): receptive field 빠르게 확장",
        "- **Residual**: 입력/출력 channel 다르면 1×1 conv로 매칭",
        "- Receptive field = 1 + 2(K-1)·Σ dilation = 1 + 2·2·(1+2+4) = 29 (L≤30까지 충분)",
        "",
        "정의는 `src/models/dl/tcn.py` 의 `TCNNet` + `TCNModel`.",
    ),
    # ----- Cell 6: 모델 인터페이스 확인 -----
    code(
        "# TCNModel은 src/models/dl/tcn.py에서 import 완료",
        "print(TCNModel.__doc__.splitlines()[0])",
        "print(f'name = {TCNModel.name}')",
    ),
    # ----- Cell 7: helper 설명 -----
    md(
        "## 헬퍼",
    ...
    code(
        ...
        "            ckpt_path = DL_MODELS_DIR / f'TCN_{regime}_{country}_{tier}.pt'",
    ...
    md(
        ...
        "- 시계열 log는 (regime, country)별 1 파일 — `log/TCN/log_TCN_{regime}_{country}.csv`",
    ),
    # ----- Cell 12: save code -----
    code(
        ...
        "    out_path = LOG_DIR / f'log_TCN_{regime}_{country}.csv'",
    ...
    md(
        ...
        "- TCN full retrain (early stop 활성)",
        "",
        "**가볍게 시작**: `REFIT_EVERY=50`으로 키우면 refit 횟수 절반.",
    ),
    # ----- Cell 14: EXPANDING LOOP -----
    code(
        "REFIT_EVERY = 20      # TCN 학습 비싸서 ML(5)보다 큰 값. 50으로 더 늘릴 수도",
        "",
        ...
        "                ckpt_path = DL_MODELS_DIR / f'TCN_{regime}_{country}_{tier}_L{L}_expanding.pt'",
    ...
    code(
        ...
        "    print('\\n=== ML(core) vs TCN(core) Full Test best ===')",
...
def build():
    ...
    out = Path(__file__).resolve().parents[1] / "notebooks" / "07_tcn.ipynb"
