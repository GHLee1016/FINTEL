...
CELLS = [
    ...
    md(
        ...
        "  ↓",
        "WeightNorm(Conv1d(F → 64, kernel=3, padding=1)) → ReLU → Dropout    # layer 1",
        "WeightNorm(Conv1d(64 → 64, kernel=3, padding=1)) → ReLU → Dropout   # layer 2",
        "WeightNorm(Conv1d(64 → 64, kernel=3, padding=1)) → ReLU → Dropout   # layer 3",
        "  ↓ (B, 64, L)",
        "out[:, :, -1] → (B, 64)        # 마지막 timestep (LSTM/TCN과 동일)",
        "  ↓",
        "nn.Linear(64 → 1) → squeeze → (B,)     # 다음날 RV 예측",
        "```",
        "",
        "- padding=kernel//2 (same-padding)으로 시퀀스 길이 L 유지",
        "- **마지막 timestep**을 head로 사용 (AvgPool은 lag-1 신호를 22~252일 평균에 묻어버려 부적합)",
        "- **WeightNorm** (TCN과 동일 패턴) — 시계열 batch 통계 불안정성 회피, 작은 dataset에 안정",
        "- Receptive field = 1 + 2·num_layers = 7 timestep (k=3 기준). L=22엔 충분, L=60/252엔 부족할 수 있음",
        "",
